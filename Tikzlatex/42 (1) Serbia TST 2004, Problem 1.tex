...
\begin{tikzpicture}[font=\small, scale=1.1]
    ...
    % --- LINGKARAN MJDP ---
    % Pusat di titik tengah JP = (3.5, 1), diameter = length(JP) = sqrt(3^2 + 2^2) = sqrt(13)
    \coordinate (CenterMJDP) at (3.5, 1);
    \draw[blue, thick, dashed] (CenterMJDP) circle ({sqrt(13)/2});

    % --- LINGKARAN INCP ---
    % Pusat di titik tengah IP = (1, 1.5), diameter = length(IP) = sqrt(2^2 + 3^2) = sqrt(13)
    \coordinate (CenterINCP) at (1, 1.5);
    \draw[teal, thick, dashed] (CenterINCP) circle ({sqrt(13)/2});
